...
if [ -z "${BITBUCKET_PR_ID}" ]; then
  ...
  OUTPUT=$(cd "${build_output_path}" && vercel -c -C --token "${VERCEL_TOKEN}" --scope ui2 -A "${VERCEL_JSON_LOCATION}/vercel.json" --prod)
  echo "errorlevel: $?"
...
HEREDOC
  ...
  OUTPUT=$(cd "${build_output_path}" && vercel -c -C --token "${VERCEL_TOKEN}" --scope ui2 -A "${VERCEL_JSON_LOCATION}/vercel.json")
  echo "errorlevel: $?"
  echo "Output from vercel:"
  echo "${OUTPUT}"
  echo "--"
