...
Any component that is likely to be needed on other pages, such as a table, button, heading, avatar, etc, should live in `/libs/ui/`. Each component in here should be exposed as a Storybook item, and is visible on [https://storybook.new.com](https://storybook.new.com) or run `yarn run nx run ui:storybook` to develop on it locally with live reloading.
...
To view the Frontend, you can either visit [https://prelive-crm.new.com](https://prelive-crm.new.com) or run `yarn start` to develop on it locally with live reloading (HMR).
...
Now add `import { } from '@ui/ui'` at top of the new page file and fill in the {} with components you need.
...
To view the Backend, you can either visit [https://backend.new.com](https://backend.new.com) or run `yarn nx serve backend` to develop on it locally with live reloading (HMR).
...
- Don't `import '../../..` across project boundaries - use `import { ... } from '@ui/ui'` instead.
...
1. Gather requirements, designs, and tests (BDD). Plan and research for an hour or so. Arrive at a rational conclusion to which npm modules, ant design components, and ui are needed.
...
1. Create a hasura/.env file:
   ...
   HASURA_GRAPHQL_ENDPOINT=https://api.new.com/
   ```
